        ...
        <div class="card-body">
            <form action="{{ route('pegawai.update', $pegawais->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <input type="date" name="tanggal" class="form-control @error('tanggal') is-invalid @enderror"
                                id="tanggal" value="{{$pegawais->tanggal}}">
                            @error('tanggal')
                            <span>{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <select name="jabatan" id="jabatan" class="form-control">
                                <option value="Sekertaris" {{ $pegawais->jabatan == 'Sekertaris' ? 'selected' : '' }}>Sekertaris</option>
                                <option value="Kepala Bidang" {{ $pegawais->jabatan == 'Kepala Bidang' ? 'selected' : '' }}>Kepala Bidang</option>
                                <option value="Bendahara" {{ $pegawais->jabatan == 'Bendahara' ? 'selected' : '' }}>Bendahara</option>
                                <option value="Staff" {{ $pegawais->jabatan == 'Staff' ? 'selected' : '' }}>Staff</option>
                            </select>
                        </div>
                    </div>
                    @error('jabatan')
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <select name="status_menikah" id="status_menikah" class="form-control">
                                <option value="Menikah" {{ $pegawais->status_menikah == 'Menikah' ? 'selected' : '' }}>Menikah</option>
                                <option value="Belum Menikah" {{ $pegawais->status_menikah == 'Belum Menikah' ? 'selected' : '' }}>Belum Menikah</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <select name="agama" id="agama" class="form-control">
                                ...
                                <option value="Katolik" {{ $pegawais->agama == 'Katolik' ? 'selected' : '' }}>Katolik</option>
                                <option value="Kristen" {{ $pegawais->agama == 'Kristen' ? 'selected' : '' }}>Kristen</option>
                ...
                <div class="form-group">
                    ...
                    <a href="{{asset('storage/'.$pegawais->file)}}" target="_blank">Lihat PDF</a>
                    @else
                    Tidak ada PDF!
                @endif
                    @error('pdf')
